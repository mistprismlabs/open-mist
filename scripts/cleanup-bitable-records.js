...
function buildTableConfigs() {
  ...
  const tables = [
    ...
    { name: "选题池",       appToken: hotAppToken,                  tableId: process.env.TOPIC_POOL_TABLE_ID, dateField: "推荐时间", retainDays: 7 },
    { name: "每日资讯简报", appToken: briefingConfig?.app_token,     tableId: process.env.BRIEFING_TABLE_ID,  dateField: "采集日期", retainDays: 30 },
    { name: "GitHub 更新",  appToken: githubConfig?.app_token,      tableId: process.env.GITHUB_TABLE_ID,    dateField: "采集日期", retainDays: 30 },
  ];
